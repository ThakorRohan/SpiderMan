...
import spiderman.modules.sql.notes_sql as sql
from spiderman import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from spiderman.__main__ import DATA_IMPORT
from spiderman.modules.helper_funcs.chat_status import user_admin
from spiderman.modules.helper_funcs.misc import build_keyboard, revert_buttons
from spiderman.modules.helper_funcs.msg_types import get_note_type
from spiderman.modules.helper_funcs.alternate import typing_action
from spiderman.modules.rules import get_rules
import spiderman.modules.sql.rules_sql as rulessql
from spiderman.modules.sql import warns_sql as warnssql
import spiderman.modules.sql.blacklist_sql as blacklistsql
from spiderman.modules.sql import disable_sql as disabledsql
from spiderman.modules.sql import cust_filters_sql as filtersql
import spiderman.modules.sql.welcome_sql as welcsql
import spiderman.modules.sql.locks_sql as locksql
from spiderman.modules.connection import connected
...
def export_data(update, context):
	...
	f=open("spiderman{}.backup".format(chat_id), "w")
	...
	try:
		...
		pass
	context.bot.sendDocument(current_chat_id, document=open('spiderman{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `spiderman-Backup` is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
	os.remove("spiderman{}.backup".format(chat_id)) # Cleaning file
