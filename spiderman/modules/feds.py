...
from spiderman import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
from spiderman.modules.helper_funcs.handlers import CMD_STARTERS
from spiderman.modules.helper_funcs.misc import is_module_loaded, send_to_list
from spiderman.modules.helper_funcs.chat_status import is_user_admin
from spiderman.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from spiderman.modules.helper_funcs.string_handling import markdown_parser
from spiderman.modules.disable import DisableAbleCommandHandler

import spiderman.modules.sql.feds_sql as sql

from spiderman.modules.connection import connected
from spiderman.modules.helper_funcs.alternate import send_message, typing_action, send_action
	...
	if args:
		if args[0] == 'json':
			...
			with BytesIO(str.encode(backups)) as output:
				output.name = "spiderman_fbanned_users.json"
				update.effective_message.reply_document(document=output, filename="spiderman_fbanned_users.json",
			...
			for users in getfban:
				...
				backups += "\n"
			with BytesIO(str.encode(backups)) as output:
				output.name = "spiderman_fbanned_users.csv"
				update.effective_message.reply_document(document=output, filename="spiderman_fbanned_users.csv",
